...
后台采用“深色业务导航 + 浅色工作区”的结构。顶栏不承担页面导航：左侧放置侧栏控制和受限宽度的全局检索，中段显示“运营工作台”上下文，右侧放置大屏快捷入口和用户菜单；其下的 `PageTabs` 承担业务页面切换。这样保留桌面端的信息密度，同时避免视觉上出现没有语义的空白列。

### 15.2 路由标签状态

```ts
interface WorkspaceTab {
  path: string        // Vue Router fullPath，详情页可各自独立保留
  title: string       // 优先取 route.meta.title
  closable: boolean   // 仪表盘为 false，其余业务页为 true
}
```

`useAppStore` 保存 `tabs` 与 `activeTabPath`，并在变更后写入 `sessionStorage`。路由守卫/布局监听在进入受保护业务路由时执行 `openTab(route)`；`/login` 与 `/bigscreen` 不参与工作区。恢复时验证每一条路径可由 Router 解析、排除公开页并按 path 去重，保证损坏的浏览器缓存不会破坏后台导航。

### 15.3 标签关闭与路由一致性

- 单击标签调用 `router.push(tab.path)`，路由变化是唯一激活态来源。
- 关闭事件必须执行 `stopPropagation`，避免关闭按钮额外触发选中。
- 仪表盘为固定首页，永远不可关闭。
- 关闭当前标签时先按被关闭标签原索引选择右侧标签，否则左侧标签，最后回退仪表盘，再进行路由跳转；关闭非当前标签不改变当前页面。
- 标签栏使用横向滚动容纳溢出标签，不压缩到不可读，也不换行挤压内容区。

### 15.4 组件边界

- `Header.vue`：顶栏操作、搜索、工作区文案、用户菜单；显式导入所有 Element Plus 图标。
- `PageTabs.vue`：渲染/切换/关闭工作区标签，调用 app store 与 router。
- `app.ts`：标签恢复、去重、打开、激活、关闭等无视觉状态。
- `router/index.ts`：所有业务路由提供中文 `meta.title`，作为标签的稳定标题来源。
- `AppLayout.vue`：按 `Header → PageTabs → RouterView` 组织右侧工作区。
...
公众号是宠主面向门店的服务入口，不是门店经营后台的自然语言入口。回调验签成功后，入口层以 `openid` 作为外部会话标识，在当前 `tenant_id` 范围内查询客户档案，并构造以下最小上下文：

```python
class PublicAccountContext(TypedDict):
    channel: Literal["wechat_public"]
    customer_facing: Literal[True]
    tenant_id: str
    thread_id: str
    openid: str
...
`channel=wechat_public` 与 `customer_facing=True` 是强制边界。该上下文只允许进入宠主服务路径：到店服务预约、预约调整/补充、宠物养护或健康咨询、人工服务引导。内部的数据分析、客户运营、库存供应链和营销内容路径在该渠道不可达，也不得出现在澄清或兜底回复中。

### 16.2 入站处理顺序

```mermaid
sequenceDiagram
    participant P as 公众号宠主
    participant W as 公众号回调
    participant S as 服务会话入口
    participant C as 客户档案
    participant O as 渐进式建档
    participant R as 宠主服务处理

    P->>W: 文本消息
    W->>S: 验签后转交(openid, tenant_id, content)
    S->>C: 在当前租户按 openid 查找客户
    alt 未匹配客户
        S->>O: 先进入建档，保留原消息和服务诉求
        O-->>P: 询问姓名/宠物名或继续收集缺失资料
    else 已匹配客户
        S->>R: 限定在宠主服务范围内处理
        R-->>P: 预约、咨询或补充信息回复
    end
...
未匹配客户的首条消息不得先进入通用意图识别。建档处理器先读取并保留原始服务诉求：若宠主同时表达预约或咨询，资料收集完成后在同一 `thread_id` 中继续该诉求，而不是要求再次描述。

### 16.3 渐进式建档状态

建档以“可服务优先、资料渐进补齐”为原则：

- 首先收集并持久化宠主姓名和宠物名称，随后创建或更新客户/宠物关联记录。
- 手机号、物种、品种是后续优先补充项；缺失字段保持为空，并以 `onboarding_pending=True` 表示待完善，不能写入事实性 `unknown`。
- 任何一轮中发现预约服务、目标宠物或时间信息，均写入该会话的待处理服务上下文。后续资料补齐后复用该上下文执行已有预约校验与预约流程。
- 已建档但资料不完整的宠主也沿用同一补充逻辑；不会因资料待完善而暴露内部能力或丢失当前服务诉求。

### 16.4 模板渲染与宠主回复

预约提取提示词包含结构化 JSON 示例。模板实现必须将静态示例与动态上下文分离，或对花括号字面量进行正确转义；只允许定义明确的动态占位变量。这样 JSON 的 `service_type` 等键不会被 `str.format` 一类插值机制解析为变量并导致异常。

公众号可见回复采用宠主服务词汇。可澄清的内容包括洗护、美容、寄养等到店服务、宠物名称、期望日期时间、养护或健康问题；不可出现数据分析、客户运营、库存、供应链、营销或其他内部能力。发生未预期异常时，入口层写入带关联标识的标准错误日志，并返回例如“抱歉，刚才没有处理好这条消息。您可以告诉我想预约的服务、宠物名称或想咨询的问题，我继续帮您处理。”的可继续对话提示；不得伪装为已受理或即将处理。

### 16.5 验证范围

实现应覆盖：含 JSON 示例的预约模板可安全渲染；未建档公众号用户在通用识别前进入建档；建档过程保留首条预约/咨询诉求；宠主澄清与异常回复不含内部经营词汇；以及已建档与未建档用户均受渠道服务边界约束。实现继续仅使用云端 LLM 的提示工程/少样本能力，不引入模型微调。
